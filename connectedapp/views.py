...
def profile_change(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('connectedapp:profile')
    else:
...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            login(request, user)
            return redirect('connectedapp:index')
    else:
...
class CourseCreateView(LoginRequiredMixin, TeacherRequiredMixin, CreateView):
    ...
    success_url = reverse_lazy('connectedapp:course_list')
...
class FeedbackCreateView(CreateView):
    ...
    def get_success_url(self):
        return reverse_lazy('connectedapp:course_detail', kwargs={'pk': self.object.course.pk})
...
class FeedbackDeleteView(DeleteView):
    ...
    success_url = reverse_lazy('connectedapp:feedback_list')
...
class CourseEnrollView(LoginRequiredMixin, RedirectView):
    def get_redirect_url(self, *args, **kwargs):
        return reverse('connectedapp:course_detail', kwargs={'pk': self.kwargs.get('course_id')})
...
class CourseUnenrollView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return redirect('connectedapp:course_detail', pk=self.course.id)
...
def chat_clear_room(request, room_name):
    ...
    return redirect('connectedapp:chat', room_name='ConnectEd')
